...
* **T8-9** Align comments API client with shared request helper. *DoD:* Replace the hand-rolled `fetch` in `lib/api/comments.ts` with the shared `requestJson` helper, preserving standardized error parsing; update any mocks or tests relying on the old implementation; ensure typings remain accurate.
