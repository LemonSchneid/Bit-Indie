...
* **T0-1** Monorepo scaffolding (apps/web, apps/api, infra/docker-compose). *DoD:* `docker-compose up` brings Postgres, MinIO, API health.
* **T0-2** FastAPI skeleton + health route + OpenAPI. *DoD:* `/docs` renders; CORS set.
* **T0-3** Next.js app boot + base layout. *DoD:* Web home renders; calls API health.
* **T0-4** DB init + Alembic baseline. *DoD:* `alembic upgrade head` creates v0 tables.

### Milestone 1 — Auth, Devs, Games (Week 2–3)

* **T1-1** NIP-07 login flow (challenge → signature). *DoD:* Can login; user in DB.
* **T1-2** Become Developer endpoint/UI. *DoD:* Dev profile saved; badge on profile.
* **T1-3** Game create/update endpoints + form. *DoD:* Draft saved; validation errors surfaced.
* **T1-4** Storage presign + upload build/cover. *DoD:* Upload to MinIO/S3 works; checksum stored.
* **T1-5** Publish checklist + UNLISTED status. *DoD:* Game visible at direct URL.

### Milestone 2 — Purchase & Download (Week 4–5)

* **T2-1** LNbits invoice creation + webhook verification. *DoD:* Invoice lifecycles recorded.
* **T2-2** Purchase flow UI (modal → QR → polling). *DoD:* Paid state unlocks download.
* **T2-3** Signed URL for downloads. *DoD:* Expiring links; audit log entry.
* **T2-4** Basic receipts (email optional later). *DoD:* Client sees receipt page.

### Milestone 3 — Comments, Reviews, Ranking (Week 6–7)

* **T3-1** Comments CRUD (free speech). *DoD:* Post/list comments per game.
* **T3-2** Verified-purchase reviews (rating gated). *DoD:* If purchase → allow rating; else text only.
* **T3-3** Helpful score calculation (backend service). *DoD:* Endpoint returns sorted reviews.
* **T3-4** UI badges (Verified Purchase, sats received). *DoD:* Visible labels; tooltip for disclosures.

### Milestone 4 — Zaps Integration (Week 8–9)

* **T4-1** Client zap buttons (Lightning Address/LNURL). *DoD:* Wallet opens; payment leaves client.
* **T4-2** Zap receipts ingestion from relays. *DoD:* On receipt, DB `zaps` row added; review helpful_score recomputed.
* **T4-3** Anti self-zap + correlation guard. *DoD:* Self zaps excluded; simple correlation rule flagged.

### Milestone 5 — Discovery & Moderation (Week 10)

* **T5-1** Unlisted → Discover auto-promotion. *DoD:* Transition on ≥1 verified purchase + ≥1 review.
* **T5-2** Admin mod queue (flags, takedown). *DoD:* Admin can unlist a game or hide content.
* **T5-3** Rate limits + PoW for low-rep. *DoD:* Comment spam rate drops; limits logged.

### Milestone 6 — Launch Hardening (Week 11–12)

* **T6-1** Refund request flow (manual payout). *DoD:* Admin can mark REFUNDED; payout recorded.
* **T6-2** Integrity dashboard (refund %, takedown %, mod hours). *DoD:* Stats page for ops.
* **T6-3** Telemetry & error alerting. *DoD:* Basic logs/alerts wired.
* **T6-4** First Featured rotation UI. *DoD:* Featured shelf renders; criteria enforced.
