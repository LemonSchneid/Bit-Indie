...
* **T8-1** Extract game drafting service for routers. *DoD:* Introduce a dedicated service that owns developer authorization, slug uniqueness checks, price validation, featured-state transitions, and release-note toggles; update the games FastAPI router endpoints to delegate to the service while keeping HTTP response handling in the router; add unit tests covering slug conflicts, price validation, and featured toggles.
